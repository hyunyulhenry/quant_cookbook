...
```{r fig.cap='네이버금융 개별종목 차트탭', fig.align='center', out.width = '100%', echo = FALSE}
knitr::include_graphics('images/crawl_practice_price.png')
```
...
```{r fig.cap='네이버금융 차트의 통신기록', fig.align='center', out.width = '100%', echo = FALSE}
...
```{r fig.cap='주가 데이터 페이지', fig.align='center', out.width = '100%', echo = FALSE}
...
```{r fig.cap='Company Guide 스냅샷 화면', fig.align='center', echo = FALSE}
knitr::include_graphics('images/crawl_practice_comp_price2.png')
```
...
```{r fig.cap='야후 파이낸스 재무제표', fig.align='center', echo = FALSE}
knitr::include_graphics('images/crawl_practice_yahoo.png')
```
...
상장주식수의 경우 **Shares Outstanding** 부분에서 찾을 수 있습니다. 해당 지점의 Xpath를 이용해 데이터를 찾으면 `r share_yahoo`가 추출됩니다. 이 중 숫자 뒤 알파벳 부분은 단위에 해당하는 부분이며, 각 문자 별 단위는 다음과 같습니다.

```{r echo = FALSE}
knitr::kable(
  data.frame(
    '알파벳' = c('M', 'B', 'T'),
    '단위' = c('백만 (Million)', '십억 (Billion)',
             '일조 (Triliion)'),
    '숫자' = c('1,000,000', '1,000,000,000',
             '1,000,000,000,000')
  ),
  booktabs = TRUE,
  caption = '발행주식수 단위'
...
knitr::kable(
  data.frame(
    round(rbind(data_value, data_value_yahoo), 2),
    row.names = c('Company Guide', '야후 파이낸스')
    ),
  booktabs = TRUE,
  caption = '두 사이트의 가치지표 비교'
)
```
