...
```{r fig.cap='밸류 팩터와 퀄리티 팩터간의 관계', fig.align='center', out.width = '50%', echo = FALSE}
...
knitr::kable(
  data.frame(
    ...
    'v2' = c('투하 자본 수익률 (Return On Capital)',
...
국내 종목들의 PBR을 통해 히스토그램을 그려보면 오른쪽으로 꼬리가 매우 긴 분포를 보이고 있습니다. 이는 PBR이 무려 `r round(max(KOR_value$PBR, na.rm = TRUE),2)`인 이상치 데이터가 존재하기 때문입니다. 이처럼 모든 팩터 지표에는 극단치 데이터가 존재하기 마련이며, 이를 처리하는 방법에 대해 알아보도록 하겠습니다.
