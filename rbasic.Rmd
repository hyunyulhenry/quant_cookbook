...
만일 숫자 뒤에 **:**을 붙이면, integer(정수) 형태의 숫자가 만들어 집니다.
...
첫번째 원소는 정수(int), 두번째 원소는 문자(chr), 세번째 원소는 논리값(logi), 네번째 원손,ㄴ 숫자(num)으로 이루어져 있습니다. 이처럼 리스트는 각 원소간 타입이나 길이가 달라도 데이터가 결합할 수 특징이 있습니다.
...
#### `dplyr` 패키지의 함수와 SQL 구문 비교

아래의 표는 `dplyr` 패키지의 함수와 SQL 구문을 비교한 표입니다. 이름부터 비슷한 것이 많으며 사용법 자체도 유사하기에, 둘 중 하나가 익숙하면 나머지도 자연스럽게 익숙해집니다.

```{r echo = FALSE}
tribble(
  ~ '작업', ~ 'dplyr', ~ 'SQL',
  '행 선택',  'df %>% <br> filter(x > n)', 'SELECT * <br>  FROM df <br> where x > n',
  '정렬', 'df %>% <br>arrange(x)', 'SELECT * <br> FROM df <br> ORDER BY x',
  '선택', 'df %>% <br> select(x)', 'SELECT x <br> FROM df',
  '열 생성', 'df %>% <br> mutate(y = f(x))', 'SELECT f(x) as y <br> FROM df',
  '요약 통계량 계산', 'df %>% <br> summarize(min(x))', 'SELECT min(x) <br> FROM df',
  '유일값', 'df %>% <br> distinct(x)', 'SELECT DISTINCT(x) <br> FROM df',
  '그룹', 'df %>% <br> group_by(x) %>% <br> summarize(n())',
          'SELECT count(*) <br> FROM df <br> GROUP BY x',
  '레프트 조인', 'a %>% <br> left_join(b, by = "x")', 'SELECT *<br>  FROM a JOIN y<br>  on a.x=b.x'
) %>%
  knitr::kable(
  booktabs = TRUE,
  align = "l",
