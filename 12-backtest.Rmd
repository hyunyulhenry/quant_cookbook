...
리밸런싱 전일 종료시점의 비중과 리밸런싱 당일 시작시점의 비중 차이의 절대값을 합하면, 포트폴리오의 회전율을 계산할 수도 있습니다. 해당 예제에서는 `r rownames(df_ret)[4]`일 종료시점의 비중인 `r as.numeric(df_ret[4, c(11,12)])`와 `r rownames(df_ret)[5]`일 시작시점의 비중인 `r as.numeric(df_ret[5, c(4,5)])`의 차이인 `r as.numeric(df_ret[4, c(11,12)] - df_ret[5, c(4,5)])`의 절대값의 합계인 `r sum(abs(as.numeric(df_ret[4, c(11,12)] - df_ret[5, c(4,5)])))`가 회전율이 됩니다.
