...
따라서 기준점으로 첫번째 리스트, 즉 삼성전자의 재무 항목을 선택하도록 하며, 총 `r length(fs_item)`개 재무 항목이 존재합니다. 해당 기준을 바탕으로 재무제표 데이터를 정리하도록 하며, 전체 항목에 대한 정리 이전에 간단한 예시로 첫번째 항목인 **`r head(fs_item)[1]`** 기준 데이터 정리를 살펴보도록 하겠습니다. 
...
위의 과정을 거치면 fs_list에 총 `r length(fs_item)`개 리스트가 생성되며, 각 리스트에는 해당 재무 항목에 대한 전 종목의 연도별 데이터가 정리되어 있습니다.
