...
```{r yahoofs, echo = FALSE}
knitr::kable(
  data.frame(
    '항목' = c('Income Statement', 'Balance Sheet', 'Cash Flow'),
    'url' = c('https://finance.yahoo.com/quote/005930.KS/financials?p=005930.KS',
              'https://finance.yahoo.com/quote/005930.KS/balance-sheet?p=005930.KS',
              'https://finance.yahoo.com/quote/005930.KS/cash-flow?p=005930.KS')
  ),
  booktabs = TRUE,
  align = "c",
  caption = '야후 파이낸스 재무제표 url'
) %>%
  kableExtra::kable_styling(latex_options = c("striped", "hold_position")) %>%
...
상장주식수의 경우 **Shares Outstanding** 부분에서 찾을 수 있습니다. 해당 지점의 Xpath를 이용해 데이터를 찾으면 `r share_yahoo`가 추출됩니다. 이 중 숫자 뒤 알파벳 부분은 단위에 해당하며, 각 문자 별 단위는 다음과 같습니다.
...
결과를 확인해보면 4가지 가치지표가 잘 계산되었습니다. Company Guide에서 구한 값 `r round(data_value, 2)` 와는 재무데이터의 차이로 인해 미세한 차이가 있지만, 이는 무시해도 될 정도입니다.
